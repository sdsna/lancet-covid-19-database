...
dataset = dataset[dataset[['sub_region_1', 'sub_region_2', 'metro_area', 'iso_3166_2_code', 'census_fips_code']].isnull().all(axis='columns')]

# Normalize countries
dataset['iso_code'] = dataset['country_region'].apply(lambda country: normalize_country(country))
...
def run_pipeline(indicator):
    ...
    column = indicator.replace('google_mobility_change_', '', 1) + '_percent_change_from_baseline'
